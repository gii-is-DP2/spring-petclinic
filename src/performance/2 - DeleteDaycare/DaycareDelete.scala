...
class DaycareDelete extends Simulation {
	...
	object DeleteDaycare{
		val deleteDaycare= exec(http("DeleteDaycare")
			.get("/daycares/1/delete"))
		.pause(8)
	}

	val scnOwners = scenario("Owners").exec(Home.home, Login.login, 
	ShowDaycares.showdaycares, ShowDaycare.showDaycare, DeleteDaycare.deleteDaycare)

	val scnAdmins = scenario("Admins").exec(Home.home, Login.login, 
	ShowDaycares.showdaycares, ShowDaycare.showDaycare, DeleteDaycare.deleteDaycare, 	
	ShowDaycare.showDaycare, DeleteDaycare.deleteDaycare)

	setUp(scnOwners.inject(rampUsers(5000) during (10 seconds)),
	scnAdmins.inject(rampUsers(5000) during (10 seconds))).protocols(httpProtocol)
	.assertions(
